...
# 耗时步骤，运行一次后，就注释掉
if(T){
  myNorm <- champ.norm(beta=myLoad1$beta,arraytype="450K",cores=5)
  dim(myNorm) 
  pD=myLoad1$pd
  save(myNorm,pD,file = 'step2_GSE36278_K27M_champ_myNorm.Rdata')
}
load(file = 'step2_GSE36278_K27M_champ_myNorm.Rdata')
...
if(T){
  ...
  pheatmap(n,show_colnames =F,show_rownames = F,
           annotation_col=ac,filename = 'GSE36278_K27M_heatmap_top1000_sd.png')
  dev.off()
  ...
  pheatmap::pheatmap(cor(exprSet),
                     annotation_col = colD,
                     show_rownames = F,
                     filename = 'GSE36278_k27M_cor_all.png')
  dev.off() 
  ...
  pheatmap::pheatmap(M,
                     show_rownames = F,
                     annotation_col = colD,
                     filename = 'GSE36278_K27M_cor_top500.png')
  dev.off() 
